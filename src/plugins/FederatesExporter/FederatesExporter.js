...
*/
    ...
    /* (string) id of the interation to add    */
    addEndJoinResign = function(name, pubSubInteractions, id) {
...
*/
    ...
    objectTraverserCheck = function(federate, object) {
      ...
      if (object.name != 'ObjectRoot' && (object.id in federate.pubSubObjects)) {
...
*/
    ...
    objectTraverserXml = function(federate, object, space) {
      ...
      if (objPubSub && (objPubSub.publish || (objPubSub.mayPublish && hasOwn))) { // object publishes

        if (objPubSub.subscribe || (objPubSub.maySubscribe && hasOwn)) { // object also subscribes
        ...
        } else { // object publishes but does not subscribe
          ...
          objModel.attributes.forEach(
            ...
            function(attr) { // also set sharing for own attributes
              ...
              } else if (!attr.inherited) { // otherwise mark the attribute as Publish if not inherited
      ...
      } else if (objPubSub && (objPubSub.subscribe || (objPubSub.maySubscribe && hasOwn))) {
        ...
        objModel.attributes.forEach(
          ...
          function(attr) {
            ...
            if (objPubSub.attribs && (attr.id in objPubSub.attribs)) { // attribute is involved in a crosscut
              ...
              if (attr.inherited) { // if not own attribute, set values and set flag to print
              ...
              if (objPubSub.attribs[attr.id].publish) { // some crosscut says publish

                if (objPubSub.attribs[attr.id].subscribe) { // some other crosscut says subscribe

                  attr.sharingXml = "PublishSubscribe";
                  objModel.sharingXml = "PublishSubscribe";

                } else { // there is no subscribe crosscut, so Publish

                  attr.sharingXml = "Publish";
                  if (objModel.sharingXml != "PublishSubscribe") { // but do not change if already PublishSubscribe
                      objModel.sharingXml = "Publish";
                  }
            ...
            } else { // no publish, subscribe, or relevant crosscut
...
*/
    ...
    interactionTraverserCheck = function(federate, interaction) {
      ...
      if (interaction.name != 'InteractionRoot') {
        ...
        if ((interaction.id in federate.pubSubInteractions) && !(interaction.basePath in federate.pubSubInteractions)) {
...
*/
    ...
    interactionTraverserXml = function(federate, interaction, space) {
      ...
      intModel.sharingXml =
        (intPubSub && intPubSub.publish) ? (intPubSub.subscribe ? "PublishSubscribe" : "Publish")
                                         : (intPubSub && intPubSub.subscribe) ? "Subscribe" : "Neither";

      intModel.deliveryXml = (interaction.delivery === "reliable") ? "HLAreliable" : "HLAbestEffort";

      intModel.orderXml = (interaction.order === "timestamp") ? "TimeStamp" : "Receive";
      ...
      interaction.children.forEach(
        ...
        function (child) {
          ...
          if (child.id in federate.pubSubInteractions) {
            intModel.children.push(interactionTraverserXml(federate, child, space + "    "));
          }
...
*/
    buildScriptGenerator = function(fedEx) { // ARGUMENTS: the FederatesExporter function object

      fedEx.fileGenerators.push(

        function (artifact, fileGeneratorCallback) {
          ...
          artifact.addFile(
            ...
            function (err) {
              if (err) {
                fileGeneratorCallback(err);
              } else {
                fileGeneratorCallback();
              }
            }
          );
...
*/
      ...
      generateFiles = function(artifact, fileGenerators, generateFilesCallback) {

        if (numberOfFileGenerators > 0) {

          fileGenerators[fileGenerators.length - numberOfFileGenerators](
            artifact,
            function(err) {
              ...
              if (numberOfFileGenerators > 0) {
                ...
                generateFiles(artifact, fileGenerators, generateFilesCallback);
          ...
          self.blobClient.saveAllArtifacts(
            ...
            function (err, hashes) {
              ...
              for (idx = 0; idx < hashes.length; idx++) {
                ...
                artifactMsg = 'Code package ' + self.blobClient.artifacts[idx].name + ' was generated with id:[' + hashes[idx] + ']';
...
*/
      ...
      finishExport = function(err) { /* ARGUMENTS (string) an error string or null (?) */
        ...
        artifact = self.blobClient.createArtifact(self.projectName.trim().replace(/\s+/g,'_') + '_generated');

        fomGenerator(self);
        buildScriptGenerator(self);

        if (self.generateExportPackages) {
          coreArtifact = self.blobClient.createArtifact('generated_Core_Files');
        }
        ...
        if (numberOfFileGenerators > 0) {
          ...
          generateFiles(
            ...
            function(err) {
              ...
              if (self.generateExportPackages && numberOfFileGenerators > 0) {
...
*/
    ...
    FederatesExporter.prototype.excludeFromVisit = function(node) { /* ARGUMENTS: a webGME node to test for being excluded */
      ...
      if (self.rootNode != node) {
        ...
        nodeTypeName = self.core.getAttribute(self.getMetaType(node),'name');
